...
import { leaveService } from '@/lib/db-helpers'
import { exportLeaveToExcel } from '@/lib/export-utils'
import { sendLeaveApprovalEmail, sendLeaveRejectionEmail } from '@/lib/email-templates'
...
export default function LeavePage() {
  ...
  const handleApprove = async (requestId: string, employeeName: string) => {
    ...
    try {
      // Update in database
      const { error } = await leaveService.updateStatus(requestId, 'approved')
      ...
      toast.success('Leave request approved successfully!', { id: loadingToast })

      // Send email notification to employee
      const approvedRequest = requests.find(r => r.id === requestId)
      if (approvedRequest) {
        try {
          const employeeEmail = `${approvedRequest.employeeId.toLowerCase().replace(/-/g, '.')}@unre.ac.pg`
          ...
          toast.success('Email notification sent!', { duration: 2000 })
        } catch (emailError) {
          console.error('Error sending email:', emailError)
          toast('Email failed - check RESEND_API_KEY', { icon: '⚠️' })
        }
      }
    } catch (error) {
      console.error('Error approving leave:', error)
      toast.error('Failed to approve leave request', { id: loadingToast })
    }
  }

  const handleReject = async (requestId: string, employeeName: string) => {
    const reason = prompt(`Provide a reason for rejecting ${employeeName}'s leave request:`)
    ...
    try {
      // Update in database
      const { error } = await leaveService.updateStatus(requestId, 'rejected')

      if (error) throw error

      // Update local state
      setRequests(requests.map(req =>
        req.id === requestId ? { ...req, status: 'rejected' as const, rejectedReason: reason } : req
      ))

      toast.success('Leave request rejected', { id: loadingToast })

      // Send email notification to employee with reason
      const rejectedRequest = requests.find(r => r.id === requestId)
      if (rejectedRequest) {
        try {
          const employeeEmail = `${rejectedRequest.employeeId.toLowerCase().replace(/-/g, '.')}@unre.ac.pg`
      ...
      toast.error('Failed to reject leave request', { id: loadingToast })
    }
  }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                onClick={() => {
                  exportLeaveToExcel(filteredRequests as any)
